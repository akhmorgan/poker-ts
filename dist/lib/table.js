...
export default class Table {
    constructor(forcedBets, numSeats = 9) {
    ...
    startHand(seat) {
        assert(!this.handInProgress(), 'Hand must not be in progress');
        assert(this._tablePlayers.filter(player => player !== null).length >= 2, 'There must be at least 2 players at the table');
        if (seat !== undefined) {
            this._button = seat;
    ...
    setAutomaticAction(seat, action) {
        assert(this.canSetAutomaticAction(seat), 'Player must be allowed to set automatic actions');
